...
//        val coverageEngine = currentSuite.coverageEngine
        val projectData = currentSuite.suites[0].getCoverageData(coverageDataManager) ?: return // TODO: show message
        ...
        val repositoryFileCoverageMap: Map<GitRepository, List<FileCoverage>> =
            classes.mapNotNull { classDate ->
                val psiFile = javaPsiFacade.findClass(classDate.name, scope)?.containingFile ?: return@mapNotNull null
                if (psiFile.fileType.isBinary) {
                    return@mapNotNull null
                }
                val lines = classDate.lines.map { it as LineData }
